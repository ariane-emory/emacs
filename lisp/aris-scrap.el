...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(--dostack-mini-forth '(1 2 :over 3))
(--dostack-mini-forth '(:rotl 1 2))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(let ((final (let
               ...
               (cl-labels
                 ((flag-is?
                    (test-flag)
                    (eq flag
                      (--valid-pipe-flag-or-nil test-flag)))
                   (set-flag!
                     (new-flag force)
                     (let
                       ((new-flag
                          (--valid-pipe-flag-or-nil new-flag t)))
                       (cond
                         ((and flag new-flag
                            (not force))
                   ...
                   (store!
                     (value)
                     (setq x value))
                   (unset-flag! nil (set-flag! nil nil)))
                 (--pipe--print (make-string 80 61)) (--pipe--print "START")
                 ...
                 (catch 'return
                   (dostack (expr body)
                     ...
                     (if (--is-pipe-command? expr)
                       ...
                       (cl-flet
                         ((ignore-next-and-unset-flag! (bool)
                            (if bool
                              (let ((next (pop!)))
                         ...
                         (let
                           ...
                           (cond
                             ...
                             (t
                               (store! result)
                               (--pipe--print "Updating var to %S." x)))))))
                   (throw 'return
                     (progn
                       (--pipe--print (make-string 80 61))
                       (--pipe--print "Because empty stack: %S" x)
                       (--pipe--print (make-string 80 61))
                       x)))))))
...
(|> ((e)) 5 double (lambda (n) (+ e 2)))


(let ((final
        (let ( (body '(5 double (lambda (n) (+ e 2))))
               (e nil)
               (var-sym 'e)
               (flag nil))
          (cl-labels
