...
(when *dm:test-aux*
  ...
  (confirm that (dm::properize-pattern!*2 '(,x ,y .  z)) returns ((\, x) (\, y) \. z))
  (confirm that (dm::properize-pattern!*2 '(,x ,y . ,z)) returns ((\, x) (\, y) \. (\, z)))
  (confirm that (dm::properize-pattern!*2 '((w y) .  z)) returns ((w y) \. z))
  (confirm that (dm::properize-pattern!*2 '((w . y) .  z)) returns ((w \. y) \. z))
  (confirm that (dm::properize-pattern!*2 '((w y) .  ,z)) returns ((w y) \. (\, z)))
  (confirm that (dm::properize-pattern!*2 '((w . y) .  ,z)) returns ((w \. y) \. (\, z)))
...
(ignore!
  ...
  ((1.1257 0 0.0)
    (1.192432 0 0.0))
  ((1.1146530000000001 0 0.0)
    (1.1943599999999999 0 0.0)
    (1.271647 0 0.0)
    (1.514934 7 0.47723699999998814)
    (1.834804 8 0.5416530000000108))

  ((1.112958 0 0.0)
    (1.1967619999999999 0 0.0))
