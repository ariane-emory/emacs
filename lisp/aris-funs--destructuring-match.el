...
(when *dm:test-match*
  (let ((*dm:verbose* nil))
    ...
    (confirm that (dm:match '(foo ,(bar integer?) ,(bar odd?)          quux) '(foo 4 4 quux))
      returns nil)
    ;;-----------------------------------------------------------------------------------------------
    (confirm that (dm:match '(... ,(bar integer? odd? (lambda (n) (> n 4))) ...) '(foo 3   quux))
      returns nil)
    (confirm that (dm:match '(... ,(bar integer? odd? (lambda (n) (> n 4))) ...) '(1 2 3 4))
      returns nil)
    (confirm that (dm:match '(... ,(bar integer? odd? (lambda (n) (> n 4))) ...) '(1 2 3 5))
      returns ((bar . 5)))
