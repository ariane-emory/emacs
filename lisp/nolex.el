...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defmacro make-pick (lst) `(lambda (&optional val var vars)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defmacro swap2 (this that &optional else)
  (if else
    `(lambda (v &rest _) (if (eq ',this v) ',that ',else))
    `(lambda (v &rest _) (if (eq ',this v) ',that v))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(confirm that (funcall (swap2 i you) 'i) returns you)
(confirm that (funcall (swap2 i you) 'you) returns you)
(confirm that (funcall (swap2 i you me) 'i) returns you)
(confirm that (funcall (swap2 i you me) 'you) returns me)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defvar *rules*
  '( ;;==============================================================================================
     ...
     ( :input:          ( ,(subject subject?) ,(modal modal?) never ,@verb
                          ,(at/about       (memq _ '(at about for with)))
                          ,(a/the a/the?) ,@things)
       :responses:
       ( ;;------------------------------------------------------------------------------------------
         ( :var-funs:   ( (subject
         ...
         ( :var-funs:   ( (subject         swap-word dup-var)
                          (subject*        (swap2 i am are))
                          (maybe-really!   pick-maybe-really)
                          (maybe-actually! pick-maybe-actually)
                          (verb            (lambda (v _ _) (add-ing (car (last v)))))
                          (maybe-when!     (cointoss 'right\ now nil))
                          )
           :response:   ( 10X ,maybe-actually ,subject ,maybe-really ,subject* ;; ,am/are
                          ,verb ,at/about ,a/the ,@things ,maybe-when \!))))
     ;;==============================================================================================
     ...
     ( :input:          ( divide ,(left integer?) by ,(right integer?))
       ...
       ( ;;------------------------------------------------------------------------------------------
         ( :var-funs:   ( (sum! (lambda (_ _ vars)
                                  (let-alist vars (setf last-result (/ .right .left))))))
           :response:   ( 21 the result is ,sum))))
     ;;==============================================================================================
     ( :input:          ( add ,(left integer?) to it)
       ...
       ( ;;------------------------------------------------------------------------------------------
         ( :var-funs:   ( (sum! (lambda (_ _ vars)
                                  (let-alist vars (setf last-result (+ .left last-result))))))
     ...
     ( :input:          ( divide it by ,(left integer?))
       ...
       ( ;;------------------------------------------------------------------------------------------
         ( :var-funs:   ( (sum! (lambda (_ _ vars)
           ...
           :response:   ( 22 okay \, now we\'ve got ,sum))))
     ;;==============================================================================================
     ( :input:          ( tell me the ,(result/total (memq _ '(result total))) again)
       :responses:
       ( ;;------------------------------------------------------------------------------------------
     ...
     ;;==============================================================================================
     ( :input:          ( ... i ... ,(feel (memq _ '(feel feeling))) ,@things )
       :responses:
       ( ;;------------------------------------------------------------------------------------------
...
(dolist (input
          '( ;; 99
             ...
             (i could never really look at the sun)
             (you could never sing for the audience)
             (you could never sing for the audience)
             (you could never sing for the audience)
             (you could never sing for the audience)
             (you could never sing for the audience)
