      ...
      (while-let ( (pat-head  (pop pat))
        ...
        (cond
          ...
          ((proper-list-p pat-head) ; recurse and merge.
            (setf alist (merge-alists alist (simple-match pat-head targ-head))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(when-let-alist (simple-match '(i ,modal-verb ,verb a ,thing) '(i have (never seen) a (red car)))
  (prndiv)
...
(let-kvp pair
  ...
  (prn "pat:   %s" .key)
  (prn "targ:  %s" .val)
  (when-let-alist (simple-match .key .val)
    (prn
